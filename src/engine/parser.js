...
export const INTENTS = [
  ...
  { id: 'use',      re: /^(服|服药|服用|吞下|敷上|用药|上药|吃点药)/ }, // 二十五期修：服药入口此前缺失——引擎有 doUse 却永远说不出"服药"
  ...
  // 二十四期夜巡修 I：寻静——打坐被"此地嘈杂"挡回后的出路，须在 go/wander 之前判定
  { id: 'xunjing',  re: /^(寻个清静处|寻清静|找清静|寻个清静|找个清静)/ },
  { id: 'craft',    re: /^(炼器|铸器|开炉|请(老师傅)?掌眼|掌眼)/ },
  { id: 'look',     re: /^(看|瞧|打量|望|观察|环顾|四下|四周)/ },
  { id: 'talk',     re: /^(和|跟|与|同)?\s*(.+?)?(聊|说说话|谈谈|攀谈|搭句话|搭话|说两句|唠唠)/ }, // 二十三期修 F：搭句话也算攀谈，单说也可
  { id: 'ask',      re: /^(问|向|找)\s*(.+)/ },
  { id: 'cultivate',re: /^(打坐|修炼|运功|行功|吐纳)/ },
  { id: 'closeddoor', re: /^(闭关|闭门|闭死关)/ }, // 2.0 第零层：大段跳岁月——光阴核
  { id: 'heal',     re: /^(养伤|将养[一二三四五六七八九十百\d]+[个月月年])/ }, // 2.0 第零层：养伤 N 月（短疗伤仍归 rest）
...
export function extractSlots(text, scene, worldData) {
  ...
  const t = text.replace(/^(去|往|到|回|离开?|问|向|找|和|跟|与|同)\s*/, '')
                .replace(/(聊|说说话|谈谈|攀谈|打听|搭句话?|说两句|唠唠)$/, '');
